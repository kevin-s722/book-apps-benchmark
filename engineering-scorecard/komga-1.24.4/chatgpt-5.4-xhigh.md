...
| Security | B | 83 | Multi-mode Spring Security, API key auth, OAuth2/OIDC, validated CORS, ControllerAdvice |
| Maintainability | B | 82 | Kotlin conciseness, DDD boundaries, event-driven design, domain lifecycle services |
| Modularity | A | 86 | Clean DDD package structure, jOOQ persistence, Spring events, ArchUnit slice isolation |
| Code Quality | A | 85 | ArchUnit architectural tests, Kotlin idioms, structured error handling, type-safe SQL |
| Testing | C | 73 | 97 backend tests with ArchUnit enforcement, but only 3 frontend tests |
| Documentation | C | 74 | CONTRIBUTING, DEVELOPING, CHANGELOG, PRIVACY, ERROR CODES, OpenAPI spec |
| Performance Design | B | 78 | jOOQ for efficient SQL, Lucene search, SQLite WAL, async events, Spring Actuator |
| Developer Experience | B | 79 | Gradle wrapper, npm scripts, 7 CI workflows, DEVELOPING guide, Conveyor packaging |
| Long-Term Sustainability | B | 84 | Spring Boot 3.5, mature versioning (v1.24.4), ArchUnit enforcement, Kotlin stability |
