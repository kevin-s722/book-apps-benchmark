...
| Total Files | ~1,169 |
| Source Files | ~356 (126 Python + 230 JS/CSS static assets) |
| Test Files | 36 |
| Languages | Python, JavaScript, HTML (Jinja2), CSS |
| Dependency Count | ~83 (requirements.txt) |
| Largest Module | cps/ (945 files including templates, translations, static) |
| Build System | setuptools (pyproject.toml), Docker |
| CI/CD Present | Yes - 6 GitHub Actions workflows |
| Containerization Present | Yes - Dockerfile (302 lines), docker-compose.yml |
| Test-to-Source Ratio | 0.28 (Python only) |
...
|----------|-------|-------|---------|
| Architecture | D+ | 45 | Flask monolith with blueprint routing, but massive god-files (admin.py: 3,115 lines) |
| Security | C | 55 | CSRF protection, rate limiting, encrypted secrets, but permissive CSP and mixed validation |
| Maintainability | D+ | 42 | Extremely large files (10,646 lines across 6 core files), no type annotations, global state |
| Modularity | C | 57 | Blueprint-based routing with services/tasks subdirectories, but tightly coupled core |
| Code Quality | C- | 52 | Functional but dense, no type hints, no linting enforcement, hard-coded paths |
| Testing | C | 58 | 36 test files with smoke/unit/integration structure, pytest infrastructure, Docker tests |
| Documentation | B | 74 | Comprehensive README (488 lines), changelogs, Docker setup, Kubernetes config |
| Performance Design | C | 56 | APScheduler for background tasks, Tornado server, but synchronous Flask architecture |
| Developer Experience | C+ | 63 | Docker-first workflow, CI with fast/slow test split, but complex container dependencies |
| Long-Term Sustainability | C | 55 | Active development (v4.0.6), but technical debt in core files, no type safety, fork-based |
...
## Weakest Attribute: Maintainability (42/100)
