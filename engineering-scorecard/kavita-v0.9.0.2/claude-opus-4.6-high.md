...
| Total Files | ~3,440 |
| Source Files | ~1,729 (964 backend C# + 765 frontend TS) |
| Test Files | 112 (backend C# only) |
| Languages | C# (.NET 10), TypeScript, SCSS, SQL |
| Dependency Count | ~134 (backend NuGet: ~66, frontend npm: ~68) |
| Largest Module | Kavita.Models (518 files), UI/Web (1,062 files) |
| Build System | dotnet/MSBuild (backend), Angular CLI/npm (frontend) |
| CI/CD Present | Yes - 9 GitHub Actions workflows |
| Containerization Present | Yes - Dockerfile with healthcheck |
| Test-to-Source Ratio | 0.11 (backend), ~0 (frontend), 0.065 overall |
...
|----------|-------|-------|---------|
| Architecture | B | 74 | Clean layered .NET solution with separate projects per concern, EF Core, Identity |
| Security | C+ | 67 | Identity + JWT + API key + OIDC, but exception leakage and broad rate limiting |
| Maintainability | C+ | 66 | Strongly typed C#, but very large model/database projects and 415 EF migrations |
| Modularity | B | 72 | Well-decomposed solution with 14 .csproj projects and clear layer boundaries |
| Code Quality | C+ | 68 | C# type safety, DI-heavy design, but inconsistent validation and exception exposure |
| Testing | D+ | 45 | Only 112 test files for 964 backend source files (0.11 ratio), no frontend tests |
| Documentation | C+ | 64 | README (116 lines), security policy, contributing guide, but limited development docs |
| Performance Design | C+ | 67 | EF Core with caching service, Hangfire for background jobs, healthcheck in Docker |
| Developer Experience | C+ | 68 | .NET CLI, Angular CLI, CI workflows, but complex multi-project setup |
| Long-Term Sustainability | C+ | 67 | .NET 10 (current), active development, but low test coverage and large migration backlog |
...
C# type safety and the DI pattern provide a solid foundation for maintainability. However, the sheer scale of the model (518 files) and database (475 files) projects makes navigation challenging. The 415 EF migrations are a maintenance concern - this many migrations increase startup time and make database rollbacks complex. Some controllers handle too many endpoints, suggesting they could benefit from decomposition. The absence of enforced code style (no .editorconfig or analyzers in CI) means formatting consistency depends on contributor discipline. The extension method pattern for DI registration is well-applied and keeps the startup clean.
...
## Best Attribute: Architecture (74/100)
## Weakest Attribute: Testing (45/100)
